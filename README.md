...
flowchart TD
    ...
    subgraph COMPARE ["Comparison"]
        R1["Classical: mAA from runtime"]
        R2["Learned: mAA from runtime"]
    end
...
| Step | Detail |
|:-----|:-------|
| Image Resize | Scale longest edge to 840 pixels |
| Grayscale | Convert BGR to grayscale for classical detection |
| Covisibility Filter | Keep pairs with covisibility >= 0.1 |
| Calibration Parsing | Load K, R, T matrices from CSV |
| Keypoint Normalization | Normalize using camera intrinsics |
...
| LoFTR | Learned | N/A (detector-free) | Transformer-based, handles low-texture regions |

### RANSAC Variants

| Method | Description |
|:-------|:------------|
| FM_RANSAC | Standard RANSAC with fixed threshold |
| FM_LMEDS | Least Median of Squares |
| USAC_MAGSAC | Marginalized sample consensus, adaptive threshold |
...
- A pose is considered accurate only if it passes both thresholds
- mAA is computed per scene and averaged across scenes

---

## Results Summary

Runtime outputs in the notebooks report:

- per-scene mAA
- average matches per scene
- average inliers per scene
- overall mAA for the current run

No fixed hardcoded benchmark values are required in this README; use notebook runtime tables/prints as source of truth.
